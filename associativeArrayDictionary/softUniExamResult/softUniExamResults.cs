...
namespace softUniExamResult
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            foreach (var (name, result) in resultPoints)   //nrp => name result pair
            {
                Console.WriteLine($"{name} | {result.Max()}");
            }

                Console.WriteLine("Submissions:");
            foreach (var (language, submissions) in countSubmission)    //language submission pair
            {
